        ...
        <v-dialog v-model="dialogSinger" max-width="960" content-class="overflow-x-hidden">
            <v-card class="pa-0 text-left relative">
                ...
                <v-card-actions class="sticky bottom secondary mx-n3 pa-3 pr-6 action-close-modal ">
                    ...
                    <v-btn color="green accent-3" text @click="closeDialogSinger">
                        Close
                    </v-btn>
                </v-card-actions>
            </v-card>
        ...
        <v-dialog v-model="dialogGenre" max-width="640" content-class="overflow-x-hidden">
            <v-card class="pa-0 text-left relative">
                ...
                <v-card-actions class="sticky bottom secondary mx-n3 pa-3 pr-6 action-close-modal ">
                    ...
                    <v-btn color="green accent-3" text @click="closeDialogGenre">
                        Close
                    </v-btn>
                </v-card-actions>
            </v-card>
        ...
        <v-col cols="12">
            <v-card class="text-left pa-4 px-6">
                ...
                <v-row>
                    <v-col cols="12" sm="8">
                        <v-form>
                            <div class="file-upload d-flex justify-center align-center py-10" :class="{'file-over': fileover}">
                                <div class="caption text-center">

                                    <v-hover v-slot:default="{ hover }">
                                        <v-btn color="info" :outlined="hover || fileover" @click="$refs.upload.click()">
                                ...
                                <input accept="audio/*" type="file" class="input-file-hidden" ref="upload" @dragover="fileover=true" @dragleave="fileover=false" @drop="fileover = false" @change="change($event)">
                            </div>
                            <transition name="height" mode="out-in">

                                <div class="d-flex my-4 tb-list-music pa-3 " :class="{play: play}" @click="play = !play" v-if="musicFile.href">
                                    <div class="d-flex list-music-content" style="width: calc(100% - 80px)">
                                        <div class="d-flex align-center justify-center  list-music-icon relative">
                                            <audio class="d-none" ref="music" :src="musicFile.href" @ended="play = false" @loadeddata="loadeddata" @timeupdate="timeUpdate"></audio>
                                            <v-img src="/img/music-placeholder-square.png" class="absolute top" width="52" aspect-ratio="1"></v-img>
                                            <div class="icon-background" :style="{background: play ? '#1f1e1e8c' : '#1f1e1e3b'}"></div>
                                        ...
                                        <div class="ml-2 pt-1 list-music-description">
                                            <div class="fs-14 text---lightColor text-truncate">
                                                {{musicFile.name}}
                                            </div>
                                            <div class="fs-13 text---blurColor">
                                                {{musicFile.size / 1000}} kB
                                            </div>
                                        </div>
                                    </div>
                                    <div class="text---blurColor fs-13 ml-1 list-music-time " style="width: 80px"><span class="text---lightColor">{{formatMusicTime(currentTime) }} </span>/ {{ formatMusicTime(duration) }}</div>
                                </div>
                            </transition>
                            ...
                            <div class="simple-form-control mt-4">
                                ...
                                <v-text-field clearable outlined solo class="simple-form-input" color="#379DDF" v-model="title" name="title" label="Title..."></v-text-field>
                            </div>
                            ...
                            <div class="simple-form-control">
                                ...
                                <v-autocomplete name="singer" class="lyric-area fs-13 " :search-input.sync="searchSinger" color="#dfdfdf" v-model="singer" :items="singers" chips solo item-text="name" item-value="id" multiple style="z-index: 100" placeholder="Choose at least singer!" :loading="isLoadingSinger">
                                    <template v-slot:selection="data">
                                        <v-chip v-bind="data.attrs" :input-value="data.selected" close @click="data.select" @click:close="removeSingers(data.item)">
                                            <v-avatar left>
                                    ...
                                    <template v-slot:item="data">
                                        ...
                                        <template v-else>
                                            <div class="d-flex align-center py-2">
                                                <v-img :src="data.item.thumbnail" elevation="2" width="32" aspect-ratio="1" class="rounded mr-2"></v-img>
                            ...
                            <div class="simple-form-control mt-4">
                                ...
                                <v-autocomplete :elevation='0' class="lyric-area fs-13 " :search-input.sync="searchGenre" color="#dfdfdf" v-model="genre" :items="genres" filled chips solo item-text="title" item-value="id" multiple style="z-index: 100" placeholder="Choose at least genre!" :loading="isLoadingGenre">
                                    <template v-slot:selection="data">
                                        <v-chip v-bind="data.attrs" :input-value="data.selected" close @click="data.select" @click:close="removeGenre(data.item)">
                                            #{{ data.item.title }}
                                        </v-chip>
                                    </template>
                                    <template v-slot:item="data">
                                        ...
                                        <template v-else>
                                            <div class="d-flex align-center pa-0 ma-0">
                                                <v-checkbox :input-value="genre.indexOf(data.item.id) >= 0" color="#dfdfdf" label="" class="pa-0 ma-0" style="height: 24px;"></v-checkbox>
                            ...
                            <div class="simple-form-control mt-4">
                                ...
                                <quill-editor class="ql-customize" v-model="lyric" ref="myQuillEditor" :options="editorOption" id="lyric">
                    ...
                    <v-col cols="12" sm="4" md="3" class="mx-auto ">
                        <div class="pa-1 mx-auto d-flex justify-center align-center relative" style="width: 200px; height: 200px;border: 3px solid #9E9E9E; border-radius: 5px;">
                            <v-icon size="160px" color="#9E9E9E" v-if="!thumbnailFile && type == 'add'">mdi-folder-image</v-icon>
                            ...
                            <input type="file" ref="thumbnail" class="input-file-hidden" @change="changeImage($event, 'thumbnail')" accept="image/*">
                        </div>
...
export default {
    ...
    created() {
        ...
        try {
            ...
            if (params.type == 'edit') {
                ...
                this.axiosInstance.post('admin/musics/findById', {
                        id: params.id
                    })
                    .then(rs => {
    ...
    methods: {
        ...
        getSinger: async function () {
            return new Promise(res => {
                ...
                setTimeout(() => {
                    ...
                    this.axiosInstance.post('admin/singers/all')
                        .then(rs => {
        ...
        add: async function () {
            ...
            rs = this.musicFile.file ? await this.uploadFile(this.musicFile.file) : null;

            let musicHref = rs ? rs[0].url : '';

            rs = this.thumbnailFile ? await this.uploadFile(this.thumbnailFile) : null;
            ...
            this.axiosInstance.post(`admin/musics/${this.type}`, dt)
                .then(rs => {
