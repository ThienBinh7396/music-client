      ...
      <v-col class="pa-0">
        <v-dialog eager v-model="dialogPlaylist" max-width="960" content-class="overflow-x-hidden">
          <v-card class="pa-0 text-left relative py-4 px-6">
            <h2
              class="mb-3"
            >{{ typeControlPlaylist == 'add' ? 'Add playlist' : `Information of ${tempPlaylist.title}` }}</h2>
            <v-row>
              <v-col cols="12" sm="4">
                <div
                  :class="responsive >= 2 ? 'mt-4' : ''"
                  class="pa-1 mx-auto d-flex justify-center align-center relative"
                  style="width: 240px; height: 240px;border: 3px solid #9E9E9E; border-radius: 5px;"
                >
                  <v-icon
                    size="160px"
                    color="#9E9E9E"
                    v-if="!tempPlaylist.thumbnail"
                  >mdi-folder-image</v-icon>
                  <div
                    class="full-width full-height"
                    v-if="thumbnailFile || typeControlPlaylist == 'edit'"
                  >
                    <v-img
                      :src="tempPlaylist.thumbnail ? tempPlaylist.thumbnail : '/img/music-placeholder-square.png'"
                      width="100%"
                      aspect-ratio="1"
                    ></v-img>
                  </div>
              ...
              <v-col cols="12" sm="8">
                ...
                <div class="simple-form-control">
                  ...
                  <v-textarea
                    ...
                    class="simple-form-input"
                    color="#379DDF"
                    v-model="tempPlaylist.description"
                    row-height="80"
                    rows="1"
                    auto-grow
                    name="title"
                ...
                <div class="simple-form-control">
                  ...
                  <v-select
                    v-model="tempPlaylist.PlaylistCategory"
                    chips
                    return-object
                    :items="categories"
                    item-text="title"
              ...
              <v-col cols="12">
                <div class="simple-form-control">
                  ...
                  <v-autocomplete
                    ...
                    item-value="id"
                    multiple
                    placeholder="Choose at least music!"
                  ...
                  >
                    <template v-slot:selection="data">
                      <v-chip
                        ...
                        close
                        @click="data.select"
                        @click:close="removeMusic(data.item)"
                      >
                        <v-avatar left>
                  ...
                  <div
                    class="d-flex my-4 tb-list-music pa-2 relative"
                    :class="{play: musicPlay.playlist == tempPlaylist.id && musicPlay.id == item.id && play}"
                    @click="controlPlay(item)"
                    v-for="(item) in tempPlaylist.musics"
                    :key="item.id"
                  >
                    <div class="d-flex list-music-content" style="width: calc(100% - 80px)">
                      <div class="d-flex align-center justify-center list-music-icon relative">
                        <v-img
                          :src="item.thumbnail ? item.thumbnail : '/img/music-placeholder-square.png' "
                          class="absolute top"
                          width="48"
                          aspect-ratio="1"
                        ></v-img>
                        <div
                          ...
                          :style="{background: musicPlay.playlist == tempPlaylist.id && musicPlay.id == item.id && play ? '#1f1e1e8c' : '#1f1e1e3b'}"
                        ></div>
                        <div
                          class="music-icon"
                          :class="{'play': musicPlay.playlist == tempPlaylist.id && musicPlay.id == item.id && play}"
                        >
                          <v-icon
                            v-if="musicPlay.playlist == tempPlaylist.id && musicPlay.id == item.id && play"
                            size="21"
                      ...
                      <div class="ml-2 mt-n1 list-music-description">
                        <div class="fs-16 text---lightColor text-truncate">{{item.title}}</div>
                        <div class="fs-12 mt-1 text---blurColor">{{item.size / 1000}} kB</div>
                      </div>
                    </div>
                    <div class="text---blurColor fs-13 ml-1 list-music-time" style="width: 80px">
                      <span
                        class="text---lightColor"
                      >{{musicPlay.playlist == tempPlaylist.id && musicPlay.id == item.id ? formatMusicTime(musicPlay.currentTime) :'0:00' }}</span>
            ...
            <v-card-actions class="sticky bottom secondary mx-n3 pa-3 pr-6 action-close-modal">
              ...
              <v-btn
                ...
                text
                @click="controlPlaylist"
              >{{ typeControlPlaylist == 'add' ? 'Add' : 'Save'}}</v-btn>
              <v-btn color="green accent-3" text @click="dialogPlaylist = false">Close</v-btn>
            </v-card-actions>
          </v-card>
        </v-dialog>
      </v-col>
          ...
          <v-row>
            <v-col cols="12">
              ...
              <v-btn
                ...
                class="text-capitalize mr-3 animated zoomIn faster"
                small
          ...
          >
            ...
            <template v-slot:item.description="{item}">
              <div
                ...
                style="width: 120px"
                v-html="item.description ? item.description : `<di class='text---blurColor'>Empty</div>`"
              ></div>
            </template>
    ...
    <v-row>
      <v-col>
        <v-card class="text-left pa-4 px-6 mt-4">
          ...
          >
            <template v-slot:item.title="{item}">
              <div style="max-width: 160px" class="text-truncate">{{item.title}}</div>
            </template>

            <template v-slot:item.thumbnail="{ item }">
              <v-img
                class="ma-2"
                lazy-src="/img/music-placeholder-square.png"
                :src="item.thumbnail"
                width="46"
                aspect-ratio="1"
              ></v-img>
            ...
            <template v-slot:item.scope="{ item }">
              <v-chip color="success" class="pb-1" small>{{ item.scope }}</v-chip>
            </template>
            <template v-slot:item.createdAt="{ item }">{{ getDate(item.createdAt) }}</template>
            <template v-slot:item.action="{item}">
              <div class="my-2">
...
export default {
  ...
  methods: {
    ...
    init: function() {
      ...
      this.axiosInstance
        .post("admin/playlistCategory/all", {
          type: "withoutUser"
        })
        .then(rs => {
    ...
    control: function() {
      ...
      if (this.type == "add") {
        this.axiosInstance
          .post("admin/playlistCategory/add", {
        ...
        this.axiosInstance
          .post("admin/playlistCategory/edit", {
    ...
    deleteCategory: function(item) {
      this.axiosInstance
        .post("admin/playlistCategory/delete", {
          id: item.id
        })
        .then(res => {
          let data = res.data;
    ...
    controlPlaylist: async function() {
      ...
      if (this.thumbnailFile) {
        ...
        url = this.tempPlaylist.thumbnail
          ? this.tempPlaylist.thumbnail
          : "/img/music-placeholder-square.png";
      }
      ...
      this.axiosInstance
        .post(`admin/playlist/${this.typeControlPlaylist}`, dt)
        .then(rs => {
          let data = rs.data;
