        ...
        <v-dialog v-model="dialog" max-width="640">
            <v-card class="py-4 px-6 text-left relative">
                <div class="singer-header ">
                    <v-img :src="showSinger ? showSinger.bannerHref : '/img/banner-placeholder.png'" min-height="180px" max-height="200px" lazy-src="/img/background-placeholder.png"></v-img>
                    <div class="d-flex my-3 align-center">
                        <v-img class="rounded flex-none" width="52" aspect-ratio="1" :src="showSinger && showSinger.thumbnail ? showSinger.thumbnail : '/img/placeholder.png'"></v-img>
                        <div class="ml-2">
                            <div class="fs-14">{{ showSinger ? showSinger.name : '' }}</div>
                            <div class="fs-13 text---blurColor">{{ showSinger ? `${showSinger.followers} followers` : '' }}</div>
                        </div>
                    </div>
                </div>
            ...
            <material-card :offset="0" :fullWidth="false">
                <template v-slot:offset>
                    <div class="d-flex grow flex-wrap v-card--material_with_icon">
                        <v-card slot="offset" class="elevation-3 v-card--material__heading v-card--material__heading_icon transition-fast" color="indigo" dark>
                            <v-icon size="40">
                        ...
                        <v-btn class="mt-4 right" absolute small right color="indigo" to="/admin/singer/control/eyJ0eXBlIjoiYWRkIn0=">
                ...
                <v-data-table :headers="headers" :items="singers" :search="search" :loading="loading" loading-text="Loading... Please wait">

                    <template v-slot:item.thumbnail="{ item }">
                        <v-img :src="item.thumbnail ? item.thumbnail : '/img/placeholder.png'" width="56" class="my-2" aspect-ratio="1"></v-img>
                    </template>

                    <template v-slot:item.followers="{ item }">
                        <v-chip small :color="getColor(item.followers)" style="min-width: 50px;height: 26px;font-size: 13px; text-align: center">{{ item.followers }}</v-chip>
                    </template>
    ...
    methods: {
        ...
        deleteSinger: function (item) {
            this.showAlert({
                ...
                if (rs.value) {
                    this.axiosInstance.post('admin/singers/delete', {
                        id: item.id
                    }).then(rs => {
                        let {
                            data
                        ...
                        if (rs.data.type == 'success') {
                            this.singers.splice(this.singers.findIndex(it => it.id == item.id), 1);
                            this.showToast({
