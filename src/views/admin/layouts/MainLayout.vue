...
import Drawer from '@/views/admin/partials/Drawer'
import AppBar from '@/views/admin/partials/AppBar'
import ViewContent from '@/views/admin/partials/ViewContent'
...
export default {
    ...
    methods: {
        ...mapMutations('app', ['showToast', 'setAxiosInstance', 'setAlert', 'setToken', 'setResponsive', 'setDetectMobile', 'setSocket']),
    ...
    created() {
        ...
        this.setToken(localStorage.getItem('_atk'));

        this.setSocket({
            //io: io('http://localhost:3000'),
            io: io('https://my-music-serve.herokuapp.com'),
            init: localStorage.getItem('_a')
        });
        console.log(localStorage.getItem('_atk'));

        this.socket.on('question', (data) => {
            console.log(data);
        });

        this.setAxiosInstance(axios.create({
    ...
    mounted() {
        ...
        this.interval = setInterval(function () {
            ...
            that.axiosInstance.post('admin/employees/checkToken', {
                token: that.token,
                store: localStorage.getItem('_atk'),
                user: localStorage.getItem('_a')
            }).then(rs => {
                let {
                    data
                } = rs;
    ...
    computed: {
        ...mapState('app', ['axiosInstance', 'responsive', 'token', 'socket']),
    }
}
</script>
